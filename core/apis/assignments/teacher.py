...
def grade_assignment(p, incoming_payload):
    ...
    gradeed_assignment = Assignment.grading(
        _id=grade_assignment_payload.id,
        grade=grade_assignment_payload.grade,
        principal=p
    )
    db.session.commit()
    gradeed_assignment_dump = AssignmentSchema().dump(gradeed_assignment)
    return APIResponse.respond(data=gradeed_assignment_dump)
